...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = let resultA = measure a 
                        resultB = measure b
...
firstHalf s = let len = length s
              in if even len
                 then take (len `div` 2) s
                 else take (len `div` 2 + 1) s
...
joinToLength n xs = let ys = [(s, length s) | s <- xs]
                     in [z1 ++ z2 | (z1, l1) <- ys, (z2, l2) <- ys, l1 + l2 == n]
